 ...
 */
define(['./HEALPixTables', '../Utils/Long', '../Utils/CircleFinder'], function (HealPixTables, Long, CircleFinder) {
    ...
    var loc2pix = function (order, phi, theta) {
        ...
        if (za <= 2. / 3.) // Equatorial region
        {
            ...
            var nSideMinusOne = Long.fromNumber(nside2 - 1);
        ...
        else // polar region, za > 2/3
        {
            ...
            nside2 * loc.sth / Math.sqrt((1. + za) / 3.);
    ...
    var HEALPixBase = {
        ...
        init: function (options) {
            ...
            try {
                for (var i = 0; i <= this.order_max; ++i) {
                    this.bn[i] = this.createBoundaries(1. << i, "NESTED");
                }
            }
        ...
        uniq2hpix: function (uniq, hpix) {
            ...
            var nside2 = HEALPixBase.pow2(hpix[0]);
            hpix[1] = uniq - 4 * nside2 * nside2;
        ...
        maxPixrad: function (order, nl4) {
            var nside2 = Math.pow(2, order);
            var va = vec3.createZPhi(2. / 3., Math.PI / nl4);
            var t1 = 1. - 1. / nside2;
            t1 *= t1;
        ...
        pix2loc: function (pix, boundaries) {
            ...
            if (jr < boundaries.nside) {
                nr = jr;
                var tmp = (nr * nr) * boundaries.fact2;
                loc.z = 1 - tmp;
                if (loc.z > 0.99) {
            ...
            var tmp = HealPixTables.jpll[xyf.face] * nr + xyf.ix - xyf.iy;
        ...
        queryMultiDisc: function (norm, rad, fact, healpixOrder) {
            ...
            for (var o = 0; o <= omax; ++o) // prepare data at the required orders
            {

                var currentBoundaries = HEALPixBase.bn[o];
            ...
            for (var i = 0; i < 12; i++) {// insert the 12 base pixels in reverse order
                stk.push(11 - i, 0);
            }
            ...
            else if (o > order) // this implies that inclusive==true
            {
                ...
                else // (zone>=1): pixel center in safety range
                {
                    if (o < omax) // check sublevels
                        for (var i = 0; i < 4; ++i) { // add children in reverse order
                            stk.push(4 * pix + 3 - i, o + 1); // add children
                        }
